...
import { loadReminderItems, saveReminderItems, type ReminderItem } from './item-edit';

const isTodayItem = (item: ReminderItem) => {
...
const shownOnDaily = (item: ReminderItem) =>
    item.kind === 'daily' || (item.kind === 'oneTime' && isTodayItem(item));
...
const dragVisibleTo = (
    ...
    const vis = items.filter(shownOnDaily);
...
export default function DailyScreen() {
    ...
    const visible = items.filter(shownOnDaily);
    ...
    const undoDone = (id: string) => {
        ...
        Alert.alert('Un-check Item', `Mark "${item.label}" as not done?`, [
            { text: 'Cancel', style: 'cancel' },
            {
                ...
                onPress: () => {
                    writeItems(items.map((one) =>
                        one.id === id ? { ...one, completed: false } : one
                    ));
                },
            },
        ]);
    ...
    const moveDrag = useCallback((y: number) => {
        ...
        if (!meta) return;
        const vis = meta.snapshot.filter(shownOnDaily);
    ...
    const snoozeLine = (item: ReminderItem) => {
