# Hand-off archive — older sessions (#25 and earlier)

> Trimmed out of `handoff.md` in session #27 to keep the working hand-off short. Full detail is preserved here (and in git history). Newest archived session first.

---

## SESSION — #26 (2026-06-28): The #25 timer-nag BLOCKING BUG is FIXED + DEVICE-VALIDATED on Patrick's phone — tapping "Done" now reliably stops the nags. Also made Gentle/Urgent optional + deselectable. All code in `app/timer.tsx`, committed by Patrick.
...
**Five edits, ALL in `app/timer.tsx`, `tsc --noEmit` clean (EXIT 0):**
...
**Docs standing-rules added this session (in `docs/session-start.md`), at Patrick's request:** One question at a time; Ask if Patrick wants to say something first; No unnecessary urgency.

**Files touched (#26):** `app/timer.tsx` (five edits); `docs/session-start.md` (three new standing rules); `docs/handoff.md` + `docs/parked-items.md`.

---

## THIS SESSION — #25 (2026-06-27): The two #25 bugs handled — Timer cancel FIXED + device-validated (committed); To-Do "Done" date FIXED (code, uncommitted, untested). Then the Timer cancel work GREW into a new gentle/urgent NAG FEATURE — BUILT + `tsc` clean, but with a KNOWN UNRESOLVED BUG (tapping the banner / Dismiss doesn't stop the nags). All uncommitted code is Patrick's to commit.

**Goal = the two bigger bugs parked for #25.** Both addressed; one grew into a feature.

**BUG 1 — Timer cancel silently failed — ✅ FIXED + DEVICE-VALIDATED + COMMITTED (Patrick).** Root cause (verified by reading `app/timer.tsx`, not assumed): `startTimer` made up a timer id from the clock (`Date.now()`) and used THAT to cancel the main alert, while the **real** identifier returned by `scheduleNotificationAsync` was thrown away; only the follow-up's real id was saved. So Cancel stopped the follow-up but **not** the main "Timer Done" alert. Fix: keep the main alert's real id and cancel with it. Patrick tested on **Simulator AND phone** — a cancelled timer no longer fires. **Committed by Patrick this session.** (A stale `.git/index.lock`, left behind by a Claude `git status` run in the sandbox, briefly blocked his commit — cleared with `rm -f /Users/patsmactop/Projects/elderlyassistant/.git/index.lock` in Terminal on the Mac. Lesson: avoid running git in the sandbox on this repo.) ⚠ This simple fix has since been **superseded in the working tree** by the nag-feature rewrite below (the `notifId` field became a `notifIds` array).

**BUG 2 — To-Do "Done" logs the tap-date, not the reminder's fire date — ✅ FIXED in code, UNCOMMITTED, NOT device-tested.** `app/_layout.tsx`, the `action === 'done'` → `source === 'todo'` branch: `completedDate` was stamped from `new Date()` (tap time). Changed to `new Date(response.notification.date * 1000)` — the moment the reminder **fired** — matching the My Day / My Week / Pets paths already in that file (the device-validated #15 approach). One-line change; **no `todo.tsx` edit needed** because the fire time is already on every delivered notification. The **on-screen** Done in `todo.tsx` (~line 285) deliberately keeps tap-time (it has no fired notification to borrow), same as My Day's accepted limitation. **Caveat recorded for Patrick:** for dated tasks with an EARLY reminder ("Week/Month before"), Done logs the fire day (days before due), not the due day; for the recurring "Day" reminders where he saw the bug, the fire day IS the right day. **Still needs a phone test** — catch an older To-Do banner, tap Done, confirm the log shows the reminder's day, not today.

**OK button — CONFIRMED working in real use, NO code change.** Patrick verified on his running appointment: a "2 days ahead" reminder, tapped **OK** → that one reminder cleared, the appointment and its other reminders untouched. Each reminder for a dated appointment is its own separate alert (verified `todo.tsx` ~lines 476–512), and OK (`_layout.tsx` ~line 135) just dismisses the one alert. Contrast: **Done** on a one-time task logs it and cancels its remaining reminders. Nothing to build — it already does what he wants.

**NEW FEATURE built this session — Timer "nag" styles + loud backup. BUILT, `tsc` clean, UNCOMMITTED, and NOT yet working (known bug below).** Design decided with Patrick one decision at a time:
- Before Start, the user picks **Gentle** (a reminder every 60s, 3 times, ~3 min — e.g. coffee) or **Urgent** (every 30s, 10 times, ~5 min — e.g. boiling water), plus a **"Loud alert if no response"** switch. Stored per-timer.
- A finished timer schedules: the main "Done" alert, then the nag series, then (if loud is on) one final insistent alert one interval after the last nag. All carry `data:{ timerId }` + category `timer`; every scheduled id is stored on the timer (`notifIds`) so they can all be cancelled on acknowledge.
- **iOS limits learned (don't re-derive):** a *repeating* alert can't fire faster than once/min, so the 30s urgent nags are scheduled as individual one-shots; truly "forever" isn't possible (64-pending cap), so urgent is capped at 5 min. Patrick wanted "every 30s forever" → settled on **every 30s for 5 min**, then the loud backup.
- **The loud alert currently uses the STANDARD tone.** A genuinely louder/distinct alarm sound needs a custom sound file bundled into the app → that's a **REBUILD** (native asset), deferred as a follow-up. Breaking through Silent / Do-Not-Disturb at full volume needs Apple's **Critical Alerts** entitlement (formal application to Apple; meant for medical/safety apps) — heavy, also deferred. Patrick chose the simple-tone path for now.
- Code: rewrote `app/timer.tsx` — added `NagStyle`/`NAG_PROFILES`, a `scheduleTimerAlerts` helper (reused by snooze), `notifIds: string[]` on the timer (replaced `notifId`/`followUpId`), a `dismiss` action now handled in the response listener, gentle/urgent buttons + a `Switch` in the UI, and made Cancel/Dismiss/tap cancel ALL of the timer's `notifIds`. Category `timer` is now registered on mount.

**⚠ KNOWN UNRESOLVED BUG — where #26 MUST start: tapping the banner — or long-press → Dismiss — does NOT stop the nags on the Simulator.** By the code a tap (DEFAULT_ACTION) or Dismiss should run `dismissTimer` → cancel every `notifId` for that timer, but it doesn't on the Simulator. **Not yet diagnosed — do NOT assume the cause.** Leading (UNCONFIRMED) theory: the cancel logic lives entirely inside the Timer **screen's** component (local `activeTimers` state + an in-screen response listener), so if the app is backgrounded or the Timer screen isn't the mounted route when the alert is tapped, nothing runs to cancel the still-pending nags (the alerts fire because iOS holds them regardless). **First steps next session:** (1) establish whether the app is foreground on the Timer screen vs backgrounded when the alerts fire/are tapped (asked Patrick, not yet answered); (2) add temporary logging in the listener + `dismissTimer` to confirm the tap even reaches JS; (3) likely fix — move the acknowledge-and-cancel into `app/_layout.tsx` (always mounted via `useLastNotificationResponse`) and make cancellation **query-based** — `getAllScheduledNotificationsAsync`, filter by `content.data.timerId`, cancel each — the dependable pattern `todo.tsx` `cancelReminders` and the `_layout.tsx` myweek/todo branches already use. Note: timer notifications carry `data:{ timerId }` but **no `source`**, so `_layout`'s current response handler (keyed on `source`) ignores them today; a timer branch keyed on `timerId` would need adding.

**Tree state for Patrick's commit:** `tsc --noEmit` clean (EXIT 0). Working tree has, UNCOMMITTED: `app/timer.tsx` (nag feature, superseding the committed simple cancel fix) and `app/_layout.tsx` (To-Do Done date fix). Committing ships the nag feature **with the known tap-to-stop bug** — the on-screen **Cancel button** still stops a timer (it cancels `notifIds` directly), but acknowledging from the banner does not. Patrick decides whether to commit now or wait until the bug is fixed.

**Files touched (#25):**
- `app/timer.tsx` — Timer cancel fix (committed), then the nag-feature rewrite (uncommitted).
- `app/_layout.tsx` — To-Do "Done" now stamps the fired date (uncommitted).
- `docs/handoff.md` + `docs/parked-items.md` — this update (pending Patrick's commit).

**Doc note:** last session's commit message said "4 easy Bugs" but #24 recorded six fixes — just a wording typo; the code is all committed, no action.
