...
list of headings is the index.

**Newest sittings are at the top. A closing session is written
here, above the rest, never at the bottom and never below the
Appendix.** The master hand-off (App-Docs/master-handoff.md)
carries only where things stand and the one-line index entry.
...
edited. The chain proper begins at #1-new; the two transition
...
#116-new was committed. Split Letters and Page are on that load and
working. He likes how they are now.
...
own. Settings tile names in the explanations below are marked bold,
the same way the page names are. The working document and the in-app
copy agree. Patrick did not want a further Guide change for the even
steps or the Middles.

**Close.** Home badges left the live handoff. That sitting is already
`#112-new`. Old load numbers 82 and 86 were dropped from what is
open. The page-slid-left line was dropped; the stack keeps the
back-drag off, and that still needs a phone check.

**Next.** Settings Guide tile names, designed-machine jobs, Birthday,
day-roll lock, and the #116-new backup and back-drag checks still
need the phone.
...
the next steps.** The second sitting implemented Daily onto the
shared list.

**#83-new was committed**, asked at the open of both sittings.
...
**The guide** is `docs/designed-implementation.md`. The first sitting
wrote the machinery, each kind, Daily on the shared list, the banner
...
**Daily onto the shared list.** Built in the second sitting.
`app/daily.tsx` is the same five-line route as Weekly. Daily's extras
live on `components/CadenceListPage.tsx`: visitors and One Time, Daily's
own add, the time-and-from row label, the Daily log, reorder of the
visible list, and Done following the item. TypeScript clean. Engine not
touched. Not on the phone.

**Next sitting.** Banner housing onto the bits. Fresh session: the
housing file and the translator are about 540 lines each, unread at
this close, and Done, Skip, Snooze, and the tap still branch on source
(Patrick, #84-new).
...
updated; the phone is still the #82-new load, testers and 72
unchanged.
...
**#79-new was committed**, asked at the open.

**Duplicate miss warning.** On open, the health-notice pop-up appeared
twice — one light (phone), one dark (app). `showHealthNotice` ran on
the first scheduler pass at launch and again when `AppState` fired
`active` on the same cold start. The first alert could run before saved
theme and popup-style preferences loaded.

What was built: `SchedulerHost` waits for `preferencesReady`, runs once
on launch, and only runs again when returning from background. Theme
sets `Appearance` before the notice can show. `notice.ts` blocks a
second alert while one is already on screen.

**iOS back-to-previous-app link.** The system link at the top was
painted wrong. Memory never set status bar style. In portrait the bar
sits over the header; in landscape over the page background.

What was built: `PageFrame` sets `StatusBar` from orientation. Setting
`UIViewControllerBasedStatusBarAppearance` to yes broke the simulator
(RCT Status Bar Manager requires **no**). It was put back to **no**.
The `PageFrame` status bar lines stay. Patrick confirmed the RCT error
cleared.

**Help.** Visible name **Help**, Home badge **?**, route still
`where.tsx`. Step 2 asks how often (no stray “every”). Cancel on steps
2 and 3 goes back one step; step 1 Cancel closes. **#70-new had said
step 1’s first choice should read Repeats every.** This sitting Patrick
said step 1 is **Repeats** and **Does not**. Checked on the simulator.

**Decision for next session.** Birthdays page — **mirror Appointments**.
Each item is a name and a date, with a yearly reminder. **On the day**
and **Day Before** are both selectable chips. Build sheet first.

**Checks.** Mac suite 301 of 301. TypeScript clean. Not on the phone.

**Session close.** `handoff.md`, `handoff-history.md`, `pending.txt`,
`in-flight.md`, and `App-Docs/master-handoff.md` refreshed. `pending.docx`
not regenerated. History is newest-on-top; Done numbers grow from the
oldest line so a new sitting does not renumber the rest.
